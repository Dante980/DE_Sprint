    ...
    try:
        ...
        for i in range(len(result["titles"]) - 1):
            ...
            if str(id) == 'None':
                ...
                query1 += "('" + result["titles"][i] + "','" + result["urls"][i] + "','" + result["categories"][i] + "','" + temp[2] + "-" + month[temp[1]] + "-" + temp[0] + "', 'vedomosti.ru'),"
...
with DAG(dag_id="vedomosti.ru", start_date=datetime(2022, 12, 6), schedule_interval='@hourly', catchup=False) as dag:
